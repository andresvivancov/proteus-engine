...
case class ChainedStreamTransformer[L <: StreamTransformer[L], R <: StreamTransformer[R]](left: L, right: R)
    ...
    new TransformDataStreamOperation[ChainedStreamTransformer[L, R], I, O] {
      override def transformDataStream(
        ...
        val intermediateResult = transformOpLeft.transformDataStream(
          ...
          transformParameters,
          input)
        transformOpRight.transformDataStream(instance.right, transformParameters, intermediateResult)
      }
    }
  }

  implicit def chainedStreamFitOperation[L <: StreamTransformer[L], R <: StreamTransformer[R], I, T](implicit
      leftFitOperation: StreamFitOperation[L, I],
      leftTransformOperation: TransformDataStreamOperation[L, I, T],
      rightFitOperation: StreamFitOperation[R, T]): StreamFitOperation[ChainedStreamTransformer[L, R], I] = {
