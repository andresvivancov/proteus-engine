...
package org.apache.flink.solma.sampling
...
import org.apache.flink.solma.sampling.SimpleReservoirSampling.ReservoirSize
        ...
        statefulStream.flatMapWithState((in, state: Option[(Long, Int, Array[T])]) => {
          ...
          state match {
            case Some(curr) => {
              ...
              val data: TraversableOnce[T] = if (changed) Seq(element) else Seq()
